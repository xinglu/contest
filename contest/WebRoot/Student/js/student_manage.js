...
function initStudentManageTool() {
	student_manage_tool = {
		init: function() {
			$.ajax({
				...
				success : function (data, response, status) {
					$("#contest_contestId_query").combobox({ 
					...
					$("#contest_contestId_query").combobox("loadData",data); 
				}
			});
		},
		reload : function () {
		...
		search: function() {
			...
			queryParams["contest.contestId"] = $("#contest_contestId_query").combobox("getValue");
				...
				$.ajax({
					...
					success : function (student, response, status) {
						...
						if (student) { 
							...
							$("#student_contest_contestId_edit").combobox({
								url:"Contest/listAll",
							    valueField:"contestId",
							    textField:"contestName",
							    panelHeight: "auto",
						        ...
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#student_contest_contestId_edit").combobox("select", student.contestPri);
									//var data = $("#student_contest_contestId_edit").combobox("getData"); 
						            //if (data.length > 0) {
						                //$("#student_contest_contestId_edit").combobox("select", data[0].contestId);
						            //}
								}
							});
							...
							$("#student_signUpTime_edit").datetimebox({
								value: student.signUpTime,
							    required: true,
							    showSeconds: true,
							});
						} else {
